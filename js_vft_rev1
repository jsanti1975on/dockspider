function verifyFuelTransactions() {
  ...
  for (var i = 2; i <= lastRow; i++) {
    ...
    var difference = amount - expectedAmount;

    // Check if the actual amount is within the tolerance (+/- 0.20)
    if (Math.abs(difference) > tolerance) {
      clearCellBackground(sheet, i, 5);
      sheet.getRange(i, 5).setBackgroundRGB(255, 255, 0);
      sheet.getRange(i, 8).setValue("Discrepancy");
    } else {
      sheet.getRange(i, 8).setValue(""); // Overwrite with nothing
    }

    // Output the difference in column G
    sheet.getRange(i, 7).setValue(difference);
  }
}
